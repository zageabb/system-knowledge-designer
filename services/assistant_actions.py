...
from models import KnowledgeDocument, KnowledgeLink
...
def plan_mutation_actions(*, question: str, evidence: list[dict], model, documents, ollama_url: str, ollama_model: str, client=None) -> MutationPlan:
    ...
    available_documents = [{"document_id": document.id, "title": document.title} for document in documents]
    prompt = f"""Suggest zero to three useful actions. Suggestions are proposals only and require explicit user confirmation.
Return JSON only: {{"action_requests":[{{"action_name":"knowledge.link_document","document_id":1,"target_type":"table or column","target_key":"exact target","reason":"why"}}]}}.
Use only the listed document IDs and exact targets. Do not request deletion, shell, filesystem, network, SQL execution, catalogue changes or any unlisted action.
...
"""
    ...
    for request in plan.action_requests:
        ...
        validate_knowledge_link(project_id=documents[0].project_id if documents else None, model=model, proposal=request)
    return plan
