...
Relationships use a full Tabulator-backed catalogue grid as well: one relationship per row, explicit one-to-many columns, table and table-scoped field dropdowns, label editing, per-row save actions, and an inline final row for adding a relationship.
